...
def read_projects(
    ...
    projects_list, count = ProjectChangeLogService(session).get_project_log_list_by_id(
        project_id, skip, limit
    )
    projects = [ProjectChangeLog.model_validate(project) for project in projects_list]
