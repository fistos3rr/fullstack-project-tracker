...
## Как бы вы масштабировали систему, если количество проектов выросло до 100 000+?
Добавить очередь вроде RabbitMQ для записи логов, чтобы при изменении проекта не ждать записи логов.
Переписать код с синхронного на асинхронный. Добавить балансировщик нагрузки например Nginx или Traefik, и за ними развернуть несколько реплик FastAPI бэка. 
Добавить лимиты запросов для пользователя. Добавить таймауты для запросов в БД.
Перенести логи в MongoDB, вместо хранения отдельной строчки на каждое изменение поля, хранить в MongoDB сразу всю операцию, структура MongoDB позволяет проще масштабироваться за счёт разбиения на кластеры. Комментарии также. 
Добавить кэширование через Redis для чтения. И кэшировать например последние измененные проекты, или проекты в статусе ACTIVE.
